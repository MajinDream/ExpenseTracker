...
final class TransactionListViewModel: ObservableObject {
    ...
    func getTransactions() {
        ...
        URLSession.shared.dataTaskPublisher(for: url)
            ...
            .sink { completion in
                ...
                case .finished:
                    print("Finished cathing transactions")
                }
    ...
    func accumulateTransactions() -> TransactionPrefixSum {
        ...
        for date in stride(from: dateInterval.start, to: today, by: 60*60*24) {
            let dailyExpense = transactions.filter { $0.dateParsed == date && $0.isExpense == true }
            let dailyTotal = dailyExpense.reduce(0) { $0 - $1.signedAmount }
            
            sum += dailyTotal
            cummulativeSum.append((date.formatted(), sum.roundedTo2Digits()))
            }
